...
function Score({ options, value, onValueChange, ends, className, ...props }: ScoreProps) {
  ...
  return (
    <div data-slot="score" className={cn("flex w-full flex-col gap-1.5", className)} {...props}>
      <div className="flex overflow-hidden rounded-lg border border-[#d7d7d3] shadow-control">
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              onClick={() => pick(opt)}
              className={cn(
                "grid h-10 flex-1 cursor-pointer place-items-center border-r border-[#e2e2de] text-base font-semibold transition-[background,color] duration-[120ms] last:border-r-0",
                selected
                  ...
                  : "text-[#26262a] bg-[linear-gradient(180deg,#ffffff,#f2f2ef)] hover:bg-[linear-gradient(180deg,#ffffff,#ecece8)]"
              )}
            >
              {opt}
            </button>
